...
begin
    ...
    begin
      ...
      repeat  // parcoure la ligne
        ...
        begin
          ...
          erreur:=pos('S2',enregistrement); // description d'un rattachement à mla branche S2 d'un aiguillage triple
  ...
  begin
    ...
    repeat
      ...
      modele:=BrancheN[i][j].BType;  //1= détecteur 2 ou 3 aiguillage
      j:=j+1;
...
begin
  recommence:
  if NivDebug=3 then AfficheDebug('Alg3 précedent='+intToSTR(prec)+' actuel='+intToSTR(actuel),clyellow);
...
begin
  if NivDebug>=2 then AfficheDebug('cherche détecteur suivant aux '+IntToSTR(prec)+'-'+intToSTR(actuel),clyellow);
  j:=0;
...
// Det1 et Det2 peuvent être séparés par des aiguillages
...
begin
  if NivDebug>=2 then AfficheDebug('cherche détecteur_suivant_El aux '+IntToSTR(el1)+'-'+intToSTR(el2),clyellow);
  // traceDet=TRUE;
  // trouver détecteur 1
  ...
  repeat // boucle d'incrément décrément
    ...
    begin
      ...
      s:=s+'- départ depuis détecteur '+IntToSTR(el1);Affiche(s,clyellow);
    end;
    AdrPrec:=el1;
    BisPrec:=FALSE;
    Bis:=FALSE;
    ...
    AdrFonc:=Adr;
    Btype:=BrancheN[branche_trouve,i].BType;  // élément suivant/précédent
    ...
    if (i=20) then
      begin Affiche('Erreur fatale 300 : Itération trop longue',clred);
  ...
  begin
    if NivDebug=3 then AfficheDebug('Pas de suivant sur séquence '+IntToSTR(el1)+' à '+intToStr(el2),clyellow);
  ...
  begin
    ...
    if (NivDebug=3) then AfficheDebug('Route trouvée',clyellow);
    i:=0;
    AdrFonc:=Adr;
    BisFonc:=FALSE;
    BisSuiv:=FALSE;
...
begin
  ...
  i:=Index_feu(adresse);
//  if (feux[i].VerrouCarre=FALSE)    // si le feu n'est pas verrouillable au carré
//  {
//    if (traceDet) {sprintf(s,"Ce signal %d n'est pas verrouillable au carré\r\n",adresse);
//               Display(s);
...
begin
  ...
  if NivDebug=3 then AfficheDebug('Pas Trouvé de feu suivant au feu Adr='+IntToSTR(ADresse),clOrange);
  //TraceDet:=false;
end;
...
// a compléter par les 4 détecteurs....
...
begin
  ...
  repeat
    ...
    if (ife=1) then begin prec:=feux[i].Adr_det1;actuel:=feux[i].Adr_el_suiv1;BisActuel:=feux[i].Btype_suiv1=5;end;  //détecteur sur le signal courant
    if (ife=2) then begin prec:=feux[i].Adr_det2;actuel:=feux[i].Adr_el_suiv2;BisActuel:=feux[i].Btype_suiv2=5;end;  // détecteur sur le signal courant
    if (ife=3) then begin prec:=feux[i].Adr_det3;actuel:=feux[i].Adr_el_suiv3;BisActuel:=feux[i].Btype_suiv3=5;end;  // détecteur sur le signal courant
    if (ife=4) then begin prec:=feux[i].Adr_det4;actuel:=feux[i].Adr_el_suiv4;BisActuel:=feux[i].Btype_suiv4=5;end;  // détecteur sur le signal courant
...
begin
  ...
  repeat // boucle d'incrément décrément
    ...
    begin
      s:='Test route en ';
      if (j=1) then s:=s+'décrément ' else s:=s+'incrément ';
      s:=s+'- départ depuis détecteur '+IntToSTR(det1);Affiche(s,clyellow);
    end;
    AdrPrec:=det1;
    BisPrec:=FALSE;
    Bis:=FALSE;
    ...
    if (i=20) then begin Affiche('Erreur fatale 200 : Itération trop longue',clred);
        AfficheDebug('Erreur fatale 200 : Itération trop longue',clred);calcul_zones_det:=3;end;
  ...
  begin
    ...
    Affiche('Mem '+IntToSTR(det2)+' à '+IntTOStr(Adr),clyellow);
    
    //if (echoLog) then writeln(fichier,'itération '+intTOstr(iteration)+' - Mem '+IntToSTR(det2)+' à '+IntToSTR(Adr),clyellow);   
    calcul_zones_det:=10;
    exit;         // réussi
  end;
end;
...
begin
  ...
  begin
    ...
    if AffSignal then Affiche('Debut du traitement général',clYellow);
    if NivDebug>=2 then AfficheDebug('Début du traitement général',clYellow);
    ...
    begin
      ...
      begin
        if AffSignal then Affiche('Le feu est verrouillable au carré',clyellow);
        if NivDebug>=1 then AfficheDebug('Le feu est verrouillable au carré',clyellow);
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  if AffSignal then Affiche('les détecteurs précédents au feu '+IntToSTR(Adrfeu)+' sont:'+intToSTR(Det_initial)+' '+intToSTR(DetPrec1)+' '+intToSTR(DetPrec2)+' '+intToSTR(DetPrec3)+' ',clyellow);
      ...
      if AffSignal then affiche('Fin de la recherche des 4 détecteurs précédents-----',clOrange);
      ...
      if AffSignal and car then Affiche('le signal a des aiguilles en talon aval mal positionnées',clYellow);
      ...
      begin
        ...
        if (NivDebug>=1) then AfficheDebug('test du sémaphore',clYellow);
        Aff_semaphore:=test_memoire_zones(AdrFeu);  // test si présence train après signal
        if Aff_Semaphore then
        begin
          if AffSignal then Affiche('train après signal-> sémaphore ou carré',clYellow);
          if (NivDebug>=1) and car then AfficheDebug('train après signal-> sémaphore ou carré',clYellow);
        ...
        begin
          ...
          begin
            if AffSignal then Affiche('Aiguille '+intToSTR(aig)+' déviée',clYellow);
            if (NivDebug>=1) then AfficheDebug('Aiguille '+intToSTR(aig)+' déviée',clYellow);
          ...
          begin
            if AffSignal then Affiche('pas d''aiguille déviée',clYellow);
            if (NivDebug>=1) then AfficheDebug('pas d''aiguille déviée',clYellow);
...
//  calcule_memoires;     // mise à jour des mémoires de zone
  begin
    Maj_feux;
    //Maj_feux;
  end
...
// supprime un évènement détecteur dans la liste
...
begin
  ...
  repeat
    ...
    repeat
      ...
      if resultat=10 then
      begin
        FormDebug.MemoEvtDet.lines.add('route traitée de '+intToSTR(det1)+' à '+IntToSTR(det2));
        uneValide:=true;
...
var Index,adr : integer;
    coche : boolean;
begin
  // récupérer le contenu du Hint dans lequel on avait stocké lors de la création de la checkbox l'index du feu
  {
...
begin
  ...
  begin
   s:='Détecteur '+intToSTR(Adresse_detecteur[j])+'=';
